...
public class BinaryTree {
    ...
    private boolean findNodePath(BinaryTree binaryTree, int element, List<Integer> curretnPath) {
        if (binaryTree == null) {
            return false;
        }
        ...
        curretnPath.add((binaryTree.value));

        boolean leftResult = findNodePath(binaryTree.left, element, curretnPath);
        if (leftResult) {
            return true;
        }
        boolean rightResult = findNodePath(binaryTree.right, element, curretnPath);

        if (rightResult) {
            return true;
        }

        curretnPath.remove((Integer.valueOf(binaryTree.value)));
